...
function SeoulMap({ setDistrict }: DistrictProps) {
  ...
  const onMouseEnterHandler = (e: React.MouseEvent, district: string) => {
    ...
    <Container>
      <svg height="100%" width="100%" xmlns="http://www.w3.org/2000/svg">
        ...
        <Path
          ...
          onMouseEnter={(e: React.MouseEvent) => onMouseEnterHandler(e, "중구")}
          onMouseLeave={onMouseLeaveHandler}
        />
        <Path
...
const Path = styled.path`
  fill: ${theme.colors.NAVYBLUE1};
  stroke-linejoin: round;
  stroke: ${theme.colors.NAVYBLUE5};
  stroke-width: 2;
  scale: 0.55;
`;
...
const Tooltip = styled.div<TooltipProps>`
  position: fixed;
  top: calc(${(props) => props.$y}px - 3rem);
  left: calc(${(props) => props.$x}px - 1rem);
  background-color: ${theme.colors.NAVYBLUE2};
